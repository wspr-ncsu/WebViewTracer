...
func GetPackageName(db *sql.DB, ln *LogInfo) (string, error) {
	log.Printf("GetPackageName %s", ln.SubmissionID)
	if ln.SubmissionID == uuid.Nil {
		return "", nil // no package name for this log but that's ok
	}

	// Get the package name from the database
	var packageName string
	err := db.QueryRow("SELECT packagename FROM submissions WHERE id = $1", ln.SubmissionID).Scan(&packageName)
