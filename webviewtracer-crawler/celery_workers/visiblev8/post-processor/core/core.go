...
func (ln *LogInfo) InsertLogfile(sqldb *sql.DB) (int, error) {
	...
	(mongo_oid, uuid, root_name, size, lines, submissionid, package_name) VALUES ($1, $2, $3, $4, $5, $6, $7)
	ON CONFLICT DO NOTHING`
		_, err := sqldb.Exec(query, ln.MongoID.String(), ln.ID.String(), ln.RootName, ln.Stats.Bytes, ln.Stats.Lines, ln.SubmissionID.String(), ln.PackageName)
...
func NewLogInfo(oid primitive.ObjectID, rootName string, submissionID uuid.UUID, packageName string) *LogInfo {
	return &LogInfo{
		MongoID:      oid,
...
func (ln *LogInfo) changeIsolate(id string, mockTime uint64) *IsolateInfo {
	iso, ok := ln.Isolates[id]
	if !ok {
		iso = NewIsolateInfo(id, mockTime)
		ln.Isolates[id] = iso
	}
	ln.World = iso
...
func (ln *LogInfo) addScript(id int, src string, code string, mockTime uint64) *ScriptInfo {
	_, ok := ln.World.Scripts[id]

	if ok {
		panic(fmt.Errorf("redefining script ID %d in isolate %s", id, ln.World.ID))
	}
	script := NewScriptInfo(ln.World, id, code, ln.World.Context.Origin, mockTime)
...
func NewIsolateInfo(id string, mockTime uint64) *IsolateInfo {
	return &IsolateInfo{
		ID:       id,
		MockTime: mockTime,
...
func NewScriptInfo(iso *IsolateInfo, id int, code string, activeOrigin *Origin, mockTime uint64) *ScriptInfo {
	return &ScriptInfo{
		Isolate:          iso,
		ID:               id,
		Code:             code,
...
func (ln *LogInfo) IngestStream(stream io.Reader, aggs ...Aggregator) error {
	...
	//ln.World.Context.MockTime = 0
	var mockTime uint64 = 0
	...
	for scan.Scan() {
		...
		if len(line) > 0 {
			...
			switch code {
			case '~':
				ln.changeIsolate(fields[0], mockTime)
			case '$':
				scriptID, err := strconv.Atoi(fields[0])
				if err != nil {
					return err
				}
				ln.addScript(scriptID, fields[1], fields[2], mockTime)
			case '!':
				scriptID, err := strconv.Atoi(fields[0])
				if err != nil {
					ln.resetContext()
			...
			default:
				for _, agg := range aggs {
					ln.World.Context.MockTime = mockTime
