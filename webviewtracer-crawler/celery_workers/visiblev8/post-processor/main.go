...
import (
	...
	flow "github.com/wspr-ncsu/visiblev8/post-processor/androidflow"
	...
	"github.com/wspr-ncsu/visiblev8/post-processor/exfil"
...
var acceptedOutputFormats = map[string]formatAggregator{
	...
	"androidflow":       {"androidflow", flow.NewAggregator},
	"exfil":             {"exfil", exfil.NewAggregator},
	"noop":              {"Noop", nullCtor},
}
...
func invoke(args []string, topLevel bool) error {
	...
	flags.StringVar(&packageName, "packagename", "", "manually specify a package name to associate with logfiles")
	...
	if packageName != "" {
		aggCtx.PackageName = packageName
	}
	...
	for inputName, inputSegments := range inputClusters {
		...
		aggCtx.Ln = core.NewLogInfo(aggCtx.LogOid, aggCtx.RootName, aggCtx.SubmissionID, aggCtx.PackageName)
