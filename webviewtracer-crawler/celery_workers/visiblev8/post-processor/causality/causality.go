...
func (agg *ScriptCausalityAggregator) DumpToPostgresql(ctx *core.AggregationContext, sqlDb *sql.DB) error {
	...
	if ctx.Formats["causality"] {
		...
		visitDomain, err := core.GetPackageName(sqlDb, ctx.Ln)
		if err != nil {
			return err
		}
		...
		for _, cr := range records {
			...
			var nullableParentUUID interface{}
			var nullableParentHash interface{}
			var nullableURL interface{}

			if cr.parent != nil {
				nullableParentUUID = cr.parent.UniqueIdentifier.String()
			}
			...
			_, err = stmt.Exec(
				logID,
				visitDomain,
				cr.child.UniqueIdentifier.String(),
				cr.genesis,
				nullableParentUUID,
