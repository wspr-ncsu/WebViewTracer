...
from app.core.database import sql_session
...
@router.post('/appsubmit', response_model=UrlSubmitResponseModel)
async def post_url_submit(request: UrlSubmitRequestModel):
    ...
    with sql_session() as session:
        ...
        if not rerun:
            ...
            submission = session.query(Submission.id).filter(Submission.packagename == packagename).first()
        ...
        if rerun or submission_id is None:
            ...
            if request.parser_config is not None:
                ...
                celery_req = celery_client.send_task(
                    name='log_parser_worker.parse_log',
                    kwargs={
                ...
                return UrlSubmitResponseModel(cached, submission_id, failed=True)
